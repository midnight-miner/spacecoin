...
const string strMessageMagic = "SpaceCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight == 1)
        nSubsidy = 40000 * COIN; // 40000 Development Funds

    if (nHeight > 1)
        nSubsidy = 0.1 * COIN / (1 + (nHeight / YEARLY_BLOCKCOUNT)); //Development Funds Confirms
	
	if (nHeight > 30)
        nSubsidy = 128 * COIN / (1 + (nHeight / YEARLY_BLOCKCOUNT)); // Launch Blocks
	
	if (nHeight > 5030)
        nSubsidy = 64 * COIN / (1 + (nHeight / YEARLY_BLOCKCOUNT)); // Late For Launch Miners
    
	if (nHeight > 10030)
        nSubsidy = 32 * COIN / (1 + (nHeight / YEARLY_BLOCKCOUNT)); // Almost Down  To Lowest Reward - Panic Miners

	if (nHeight > 15030)
        nSubsidy = 16 * COIN / (1 + (nHeight / YEARLY_BLOCKCOUNT)); // Basic Blocks >>>> Community Miners <<<< Our Favorite Group! Thank you for supporting us!
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int nHeight)
{
    ...
    nSubsidy = 32 * COIN / (1 + (nHeight / YEARLY_BLOCKCOUNT)); //Hardcoded POS reward
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SpaceCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
*/
        ...
        block.nVersion = 1;
        block.nTime    = 1440969000;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 993793;

        //// debug print
        assert(block.hashMerkleRoot == uint256("0xcaaefdf8ae4552305335a095069a3d90bcb06b576bd9df45a5e77bbabfe7cff3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf4, 0xf2, 0xf9, 0xfb };
